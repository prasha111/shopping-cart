...
const priceArray = [
  { p: 1, low: "0", high: "100" },
  { p: 2, low: "100", high: "200" },
  { p: 3, low: "200", high: "300" },
  { p: 4, low: "300", high: "500" },
  { p: 5, low: "500", high: "1000" },
];
...
function SideSection({ filter = {}, setFilter = () => {} }) {
  ...
  const handleFilter = (type, value) => {
    ...
    if (filter?.[type]?.includes(value)) {
      ...
      some = {
        ...filter,
        [type]: filter?.[type]?.filter((check) => check !== value),
      };
    } else {
      some = {
        ...filter,
    ...
    <div className="w-[30%] max-w-[300px] flex items-end mr-2 flex-col">
      <div className="flex flex-col  w-[200px] pb-2 border-b-[1px]">
        <h1 className="flex flex-row justify-between ">
          ...
          <span id="category" name="category" onClick={side}>
            ^
          </span>
        </h1>
        {sideOpen.category && (
            ...
            {data?.map((categ, index) => {
              return (
                <div className="flex mt-3 gap-4">
                  <input
                    onClick={() => {
                      handleFilter("category", categ);
                    }}
                    checked={filter?.["category"]?.includes(categ)}
                    type="checkbox"
                  />
      ...
      <div className="flex flex-col mt-4  w-[200px] pb-2 border-b-[1px]">
        <h1 className="flex flex-row justify-between ">
          <span className="font-400">Prices</span>
          <span id="price" onClick={side}>
            ^
          </span>
        </h1>
        {sideOpen?.price && (
      ...
      <div className="flex flex-col mt-4  w-[200px] pb-2 border-b-[1px]">
        <h1 className="flex flex-row justify-between ">
          <span className="font-400">Ratings</span>
          <span id="rating" onClick={side} >^</span>
        </h1>
        {sideOpen?.rating && <div>
          {new Array(5).fill("")?.map((rate, index) => {
            return (
              <div className="flex mt-3 gap-4">
                <input
                  onClick={() => {
                    handleFilter("rating", index + 1);
                  }}
                  checked={filter?.["rating"]?.includes(index + 1)}
                  type="radio"
                />
                <span className="text-[14px]">
      ...
      <div className="flex w-[200px] mt-4 flex-row justify-start items-start">
        <button
          onClick={handleClear}
          className="border border-gray-200 text-black p-[10px] rounded-md"
        >
          Clean Filters
        </button>
      </div>
      {filter?.category &&
        filter?.category.map((filterr, index) => {
          return <span className="text-[10px] bg-gray-200">{filterr}</span>;
        })}
      {filter?.price &&
        filter?.price.map((price, index) => {
          return (
            <span className="text-[10px] bg-gray-200">
              {price.low}- {price.high}
            </span>
          );
        })}
      {filter?.rating &&
        filter?.rating.map((rating, index) => {
          return <span className="text-[10px] bg-gray-200">{rating}</span>;
        })}
    </div>
  );
}
